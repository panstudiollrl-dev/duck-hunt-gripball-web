...
排隊修好之後，狗變成幾乎每次擊落都出來，而四個人的擊落次數是四倍 —— 本來想當獎勵的東西變成
常態，就不再是獎勵了（Pan 2026-08-06：「狗太常出來了 可以隨機的間歇式的偶爾出現就好」）。

兩道閘門，在**排隊之前**就決定，所以被拒絕的鴨子根本不會佔到隊伍位置：

| 常數 | 值 | 作用 |
| --- | --- | --- |
| `DOG_PRESENT_CHANCE` | 0.22 | 每一隻被打到的鴨子擲一次骰 |
| `DOG_PRESENT_MIN_GAP_SEC` | 12.0 | 距離上一次狗演完至少要隔這麼久 |

為什麼需要第二道：純機率會**成串**。0.22 之下連續兩次撞出來的機率不低，而那看起來像壞掉而不像
隨機。間隔是從**演完**算起（動畫本身就佔約 1.7 秒），而且已經排進隊伍的也算「即將發生的狗」，
一併關門 —— 四個人同時打下鴨子時，不能兩隻都過關。

注意這是**每隻鴨子**擲一次而不是每秒，所以四個人玩的時候骰的次數是四倍，看到狗的頻率也接近
四倍；`DOG_PRESENT_MIN_GAP_SEC` 就是壓住這件事的那道閘門。想更常／更少看到狗，改 `CHANCE`；
想改成「不要連著出現」的手感，改 `MIN_GAP_SEC`。
