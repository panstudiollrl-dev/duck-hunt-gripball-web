...
5. 畫面上固定 2 隻鴨子，**所有人搶同一群**，誰先打到就是誰的分（見「共搶模式」）。
   甩動 Gripball 開槍；打空會被鎖 1.2 秒不能再開。
...
Pan 2026-08-06：「目前有點太無聊 或許可以設計讓遊戲比較有競爭 比較有趣的機制」。

會無聊是**結構問題**，不是缺特效。以前每位玩家有自己的鴨子、自己的重生時鐘、自己的一條 lane，
所以四個人其實是**四場並排的單人遊戲** —— 你做的任何事都影響不到別人，那就沒有東西可以搶。

現在畫面上只有 `PARTY_SHARED_DUCK_COUNT`（2）隻鴨子，**不管幾個人玩都是 2 隻**。所有人都可以
瞄任何一隻，誰先打到就是誰的分；同一隻鴨子上別人慢一步的那一槍算失手。

鴨子頭上的號碼拿掉了 —— 那個號碼的意思是「這隻是你的」，在共搶模式下那是假訊息。

### 瞄準是自動的，那競爭標準在哪裡

Pan 的追問（2026-08-06）正中要害。自動追蹤拿掉的是**準度**，不是競爭。剩下四個軸：

| 軸 | 怎麼比 |
| --- | --- |
| 握多用力 | 追蹤速度隨握力縮放，而且是各自量程的比例（`pressPeak`），小孩和大人都到得了 1.0 |
| 什麼時候甩 | 追蹤只把準心帶進範圍，**顫音收成靜音**才是「現在這一槍會中」。早了是空槍，晚了鴨子被搶走 |
| 搶哪一隻 | 追蹤鎖的是**離你準心最近**的那隻，所以上一槍把你留在哪，決定你下一隻搶得到什麼 |
| 不要亂開槍 | 見下面的失手鎖定 |

第三個軸是故意這樣設計的：**不會**幫你避開別人已經在追的鴨子。兩支準心撞向同一隻鴨子就是那場
競賽，替其中一個轉向等於遊戲代替玩家做決定。

### 失手鎖定

Party Mode 的子彈實際上是無限的（`max_bullets = party_player_count * 99`），所以**如果失手沒有
代價，最佳策略就是照著冷卻時間狂甩** —— 上面四個軸會全部塌成比誰手抖得快，共搶會比原本更糟。

打空（含慢一步打到已經死掉的鴨子）會鎖 `PARTY_MISS_LOCK_SEC`（1.2 秒）不能開槍，而且**準心會
變半透明、停止追蹤**。

為什麼是鎖定而不是扣分：扣分和有限彈藥都會讓落後的人越陷越深，然後放棄；鎖定是即時的、看得見的，
而且剛好只懲罰狂甩這個策略。看得見很重要 —— 看不見的懲罰跟「遊戲吃掉我的輸入」分不出來，玩家會
去檢查球是不是壞了，而不是學會等時機。

### 每個人要看得到自己的分數

上方一排 `P1 / P2 / ...` 是各自的分數（`_create_party_score_hud`）。共搶只有在**進行中就看得到
誰領先**的時候才是競賽 —— 畫面上只有一個總分的話，四個人打同一群鴨子卻不知道自己是不是贏，那每
一隻鴨子都沒有賭注。被鎖定的人分數會一起變灰。

（總分 `score_current` 還是照樣累加，因為 highscore 排行榜是一整場一個數字。）

### 順手修掉：1 號因為 2 號的鴨子被打到而不能開槍

Pan 2026-08-06：「如果2號鴨子被打到，1號會暫時無法發射，等新的2號鴨子出現才能再開槍 我覺得這是
嚴重的錯誤」。

跟上面是**同一行程式碼**。開槍以前要先查 `party_ducks.get(player_id)`（「你自己的鴨子還活著嗎」），
而 `party_ducks` 是**用 player 當 key** 的。兩顆球的時候 player key 是 0 和 1 —— 剛好就是現在
slot 用的那兩個數字。一隻鴨子死掉會清掉一個 key，另一位玩家的下一槍去查就查到空的，然後就只有
一聲空響，直到約 0.45 秒後重生。

這個形狀的 bug 回不來了：**開槍完全不再用開槍的人去查任何東西**。`_party_duck_at()` 掃整群鴨子，
所以別人剛打到什麼跟你能不能開槍無關。`test_intro_and_dog.js` 直接守「不可以出現
`party_ducks.get(player_id)` 這個查法」。

### 顫音要重新上膛

`party_tone_spent` 的用途是「到了就安靜」，一次握持只換一次接近音，不然按著不放會變成持續嗡嗡聲。
共搶模式多了一種情況：**玩家沒放手，但準心被交給一隻新的鴨子**（他自己打掉了、或被別人搶走）。
那段新的接近沒有顫音的話，剛好是最需要用顫音抓時機的那一段。所以 `_update_party_tracking` 會
比對 `_get_gripball_player_target_key()`（鴨子的 instance id，不能用座標 —— 兩隻鴨子會經過同一點），
換目標就重新上膛。
...
node tools/test_intro_and_dog.js   # 開頭畫面的出路、狗叼鴨子、共搶模式與失手鎖定
```
